...
public class Debris : MonoBehaviour
{
    ...
    private void Update()
    {
        //Check to see if health is 0 so that movement can be set true
        if (health <= 0)
        {
            ...
            else if (gameManager.CurrentItems <= 0)
            {
    ...
    {
        //Bounces of rocks when collides
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Multiplier"))
        {
            ...
            duplicate.GetComponent<Debris>().StartCoroutine(AnimationReset());
